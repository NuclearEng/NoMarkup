...
func meshServerOptions(opts []grpc.ServerOption) ([]grpc.ServerOption, error) {
	...
	if cfg.Enabled {
		slog.Info("gRPC mesh mTLS enabled for chat server")
	} else {
		slog.Warn("gRPC mesh mTLS disabled; chat server accepts insecure credentials")
	}
	if al := grpmtls.PeerAllowlistFromEnv(); len(al) > 0 {
		slog.Info("gRPC mesh peer allowlist enabled for chat server", "count", len(al))
	}
