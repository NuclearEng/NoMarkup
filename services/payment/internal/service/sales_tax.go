...
// Sales tax v1 — STATIC state-rate lookup table.
//
// Source: Tax Foundation, "State and Local Sales Tax Rates, 2024"
// (https://taxfoundation.org/data/all/state/2024-sales-taxes/), accessed
// 2026-04-25. These are STATE-LEVEL combined rates only — local/county
// surtaxes are intentionally out of scope for v1 because they require a
// zip-to-jurisdiction mapping we don't have on-platform yet.
//
// Data freshness: rates are loaded into memory at compile time; the table
// must be reviewed annually (every Q1) because state legislatures adjust
// rates frequently. There is NO live integration with Avalara/TaxJar in v1
// — see PLAN §6.x for the v2 roadmap.
//
// Tax computation policy:
//   - Tax is computed on the bid amount only (subtotal). Platform fees are
//     not taxed because they are platform revenue, not part of the goods
//     sale per se.
//   - Tax is rounded to the nearest cent (banker's-rounding-via-int64).
//   - Five states with 0% state sales tax (AK, DE, MT, NH, OR) return 0.
//   - Unknown state codes default to 0 with a hint in the logs (callers
//     should treat unknown codes as "non-collecting" for v1, NOT as an
//     error, because the alternative is rejecting orders that come in
//     from territories we haven't mapped yet).
//
// Zip-code to state mapping is done by the caller via stateFromZip (a tiny
// hard-coded prefix map, also v1-static; see comment on that function).

// stateSalesTaxRate2024 is the state-level combined sales tax rate as of
// 2024 from the Tax Foundation. Stored as float64 because the precision
// loss (max ~1e-15 over the 0.0..0.1 range) is well below a cent on any
// realistic transaction (< $1B).
//
// Update cadence: review every January using Tax Foundation's annual update.
var stateSalesTaxRate2024 = map[string]float64{
	"AL": 0.0400, // Alabama
	...
	"FL": 0.0600, // Florida
	"GA": 0.0400, // Georgia
	"HI": 0.0400, // Hawaii (GET, treated as sales tax)
	"ID": 0.0600, // Idaho
	"IL": 0.0625, // Illinois
	"IN": 0.0700, // Indiana
	"IA": 0.0600, // Iowa
...
// StateTaxRate returns the state sales tax rate for a 2-letter state code,
// or 0 if the state is unknown or non-collecting.
func StateTaxRate(stateCode string) float64 {
	rate, ok := stateSalesTaxRate2024[strings.ToUpper(strings.TrimSpace(stateCode))]
	if !ok {
		return 0
	}
	return rate
...
// ComputeTaxCents returns the sales tax in cents for the given subtotal and
// state code. Rounds to the nearest cent (half-up). Returns 0 for invalid
// or non-collecting states.
//
// Hot path: this runs on every charge. Keep allocation-free.
func ComputeTaxCents(subtotalCents int64, stateCode string) int64 {
	if subtotalCents <= 0 {
		return 0
	}
	rate := StateTaxRate(stateCode)
	if rate <= 0 {
		return 0
	}
...
// one or more 3-digit zip prefixes; we look up the zip's first 3 digits.
// This is good enough for tax computation in 99%+ of US zips because state
// boundaries don't split a 3-digit zip prefix.
//
// Source: USPS ZIP Code prefix tables (public). For v1 we accept that:
//   - Some prefixes (notably 005, 09x military) are intentionally not in
//     the map and resolve to "" (no tax).
//   - Multi-state ZCTAs (very rare; ~zero in practice for our pickup-only
//     model) resolve to one canonical state.
//
// v2 plan: replace with a real zip-to-state DB or Avalara lookup.
var zipPrefixToState = map[string]string{
	// 010-027  MA, RI
	...
	// 200, 202-205  DC (200=govt; 201=VA, 220-246=VA)
...
// StateFromZip returns the 2-letter state code for a US zip code, or "" if
// the zip is unknown/non-US. Only the first 5 digits of zip are inspected.
func StateFromZip(zip string) string {
	...
	prefix := zip[:3]
	return zipPrefixToState[prefix]
}

// ComputeTaxCentsForZip is a convenience wrapper that resolves zip -> state
// before computing tax. Used by the listing-charge service when it has the
// pickup zip code but not yet the state.
func ComputeTaxCentsForZip(subtotalCents int64, zip string) (state string, taxCents int64) {
	state = StateFromZip(zip)
	taxCents = ComputeTaxCents(subtotalCents, state)
	return state, taxCents
}
