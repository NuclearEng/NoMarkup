...
func meshServerOptions(opts []grpc.ServerOption) ([]grpc.ServerOption, error) {
	...
	if cfg.Enabled {
		slog.Info("gRPC mesh mTLS enabled for notification server")
	} else {
		slog.Warn("gRPC mesh mTLS disabled; notification server accepts insecure credentials")
	}
	if al := grpmtls.PeerAllowlistFromEnv(); len(al) > 0 {
		slog.Info("gRPC mesh peer allowlist enabled for notification server", "count", len(al))
	}
