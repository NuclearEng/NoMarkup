...
// for the at-rest PII columns the job service owns: jobs.service_address (a
// CUSTOMER HOME address) and jobs.service_location_encrypted (the exact
// service point), both added to the inventory by migration 104. It also reads
// properties.location_encrypted (migration 105) when deriving a job's service
// point from a linked property.
//
// This is intentionally a sibling implementation rather than an import of
// services/user, matching the copies already carried by gateway/internal/crypto
// and services/payment/internal/crypto: the modules deliberately do not depend
// on each other's package trees, and the wire format is mirrored byte for byte
// so database/cmd/encrypt-pii can produce and consume rows interchangeably.
//
// Storage format is base64(nonce || ciphertext)
