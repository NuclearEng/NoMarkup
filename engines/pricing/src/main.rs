...
/// multiplexes every concurrent pricing call over one connection, so a single
/// bad request would take out every in-flight fair-price lookup with it.
///
...
/// in `deploy/k8s/base/pricing/deployment.yaml`), so a pod that stops answering
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let shutdown = async move {
        ...
        tracing::info!("pricing engine draining in-flight requests");
    };
