...
/// in `deploy/k8s/base/imaging/deployment.yaml`), so a pod that stops answering
...
async fn main() -> ExitCode {
    ...
    let code = match run().await {
        ...
        Err(e) => {
            let detail = format!("{e:#}");
            tracing::error!(error = detail.as_str(), "imaging engine exited with error");
            ExitCode::FAILURE
        }
    };
...
async fn run() -> anyhow::Result<()> {
    ...
    // S3_ENDPOINT defaulted to the local MinIO URL, so a production pod with no
    // S3 config booted green and only failed at the first upload.
    let endpoint = require_env("S3_ENDPOINT", environment, "http://localhost:9000")?;
    let public_url =
    ...
    let shutdown = async move {
        ...
        health_reporter
            .set_not_serving::<ImagingServiceServer<ImagingServiceImpl>>()
            .await;
        ...
        tracing::info!("imaging engine draining in-flight requests");
    };

    let addr = format!("0.0.0.0:{port}").parse()?;
    tracing::info!(
        environment = environment.as_str(),
        "imaging engine starting on {}",
        addr
    );
    ...
    tracing::info!("imaging engine shut down cleanly");
    Ok(())
}
...
mod config_tests {
    ...
    const DEV_DEFAULT: &str = "http://localhost:9000";
    const KEY: &str = "S3_ENDPOINT";
