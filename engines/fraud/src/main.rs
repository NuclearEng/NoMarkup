...
/// in `deploy/k8s/base/fraud/deployment.yaml`), so a pod that stops answering
...
async fn main() -> ExitCode {
    engine_telemetry::init("fraud-engine");

    let code = match run().await {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            let detail = format!("{e:#}");
            tracing::error!(error = detail.as_str(), "fraud engine exited with error");
            ExitCode::FAILURE
        }
    };
...
async fn run() -> anyhow::Result<()> {
    ...
    if let Ok(metrics_port) = std::env::var("FRAUD_METRICS_PORT") {
        match format!("0.0.0.0:{metrics_port}").parse() {
            Ok(metrics_addr) => {
                tokio::spawn(async move {
                    if let Err(e) = crate::metrics::serve_metrics(metrics_addr).await {
                        tracing::warn!(error = %e, "fraud metrics server exited");
                    }
                });
            }
            Err(e) => {
                tracing::warn!(error = %e, port = %metrics_port, "invalid FRAUD_METRICS_PORT, metrics disabled");
            }
        }
    } else {
        tracing::info!("FRAUD_METRICS_PORT not set, /metrics endpoint disabled");
    }
    ...
    health_reporter
        .set_serving::<FraudServiceServer<FraudServiceImpl>>()
        .await;
    ...
    let shutdown = async move {
        ...
        health_reporter
            .set_not_serving::<FraudServiceServer<FraudServiceImpl>>()
            .await;
        ...
        tracing::info!("fraud engine draining in-flight requests");
    };

    tracing::info!(
        environment = environment.as_str(),
        "fraud engine starting on {}",
        addr
    );
    ...
    tracing::info!("fraud engine shut down cleanly");
    Ok(())
}
