//! Prometheus metrics for the underwriting engine.
//!
//! Exposes `underwriting_request_duration_seconds` and
//! `underwriting_requests_total` on a separate HTTP listener. Skipped when
//! `UNDERWRITING_METRICS_PORT` is unset. Uses the `prometheus` / `hyper`
//! deps that were previously declared but never referenced.
...
const BUCKETS: &[f64] = &[0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5];
...
pub static REQUEST_DURATION: LazyLock<Histogram> = LazyLock::new(|| {
    let h = Histogram::with_opts(
        HistogramOpts::new(
            "underwriting_request_duration_seconds",
            "End-to-end latency of Underwrite through the underwriting engine.",
        )
...
pub static REQUESTS_TOTAL: LazyLock<IntCounter> = LazyLock::new(|| {
    let c = IntCounter::new(
        ...
        "Total number of underwriting decisions served.",
    )
...
pub async fn serve_metrics(addr: SocketAddr) -> std::io::Result<()> {
    ...
    tracing::info!(addr = %addr, "underwriting metrics server listening");
...
mod tests {
    ...
    fn metrics_register_without_panic() {
        ...
        let names: Vec<&str> = families
            ...
            .collect();
        assert!(names.contains(&"underwriting_request_duration_seconds"));
        assert!(names.contains(&"underwriting_requests_total"));
    }
}
