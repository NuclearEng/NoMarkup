...
//!
//! This engine had **no** panic boundary at all until the resilience fix: a
//! panic while computing a credit-limit decision unwound out of the handler and killed the
//! whole HTTP/2 connection, taking every other in-flight call multiplexed on
//! it down too (the workspace does not set `panic = "abort"`).
//!
...
async fn catch_panic_layer_converts_handler_panic_to_grpc_internal() {
    ...
    let req = Request::builder()
        ...
        .uri("/nomarkup.underwriting.v1.UnderwritingService/Underwrite")
        .header(http::header::CONTENT_TYPE, "application/grpc")
        .body(tonic::body::empty_body())
