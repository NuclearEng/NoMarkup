...
| **Prometheus + Grafana + Alertmanager** | in-cluster (`monitoring` ns) | **Optional** kustomize root `deploy/monitoring/` (not part of this base/overlays path). Reuses `prometheus/{prometheus,alerts}.yml`, Alertmanager blackhole config, Grafana datasource → Prometheus + dashboards. Apply: `kubectl apply -k deploy/monitoring` after creating `nomarkup-metrics-token`. See [`docs/operations/monitoring-stack.md`](../../docs/operations/monitoring-stack.md) (OPS-10 Done). |
