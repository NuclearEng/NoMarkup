...
export function AuctionArena({ job, isProvider, isJobOwner }: AuctionArenaProps) {
  ...
  // Use WebSocket data if connected, fall back to REST
  const displayLowest = currentLowest || auctionState?.lowest_bid_cents || 0;
  const displayBidCount = bidCount || auctionState?.bid_count || 0;
  const displayEndsAt = auctionEndsAt || auctionState?.auction_ends_at || job.auction_ends_at;
  const displaySnipeCount = snipeExtensionCount || auctionState?.snipe_extension_count || 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-lg border bg-card p-4">
          <p className="text-sm text-muted-foreground">Current Lowest</p>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-3xl font-bold">{String(displayBidCount)}</p>
