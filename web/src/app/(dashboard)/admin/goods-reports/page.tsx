...
import { type AdminReport, useAdminReports, useResolveReport } from '@/hooks/useAdmin';
...
export default function AdminGoodsReportsPage() {
  const [statusFilter, setStatusFilter] = useState<string | undefined>('open');
  const [page, setPage] = useState(1);
  const [target, setTarget] = useState<{ report: AdminReport; action: ResolveAction } | null>(
    null,
  );
  ...
  const columns: Column<AdminReport>[] = [
    {
      ...
      render: (r) => (
        <div>
          <p className="font-medium">{r.listing_title || r.listing_id.slice(0, 8)}</p>
  ...
  if (isError) {
    return (
      <div className="space-y-6">
        <h1 className="gold-text text-2xl font-bold tracking-tight">Goods Reports</h1>
        <EmptyState
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-zinc-300">
            Buyer-flagged listings: stolen, counterfeit, prohibited items.
          </p>
        </div>
        ...
        <ActionConfirmDialog
          ...
          description={
            target?.action === 'actioned'
              ? 'This will mark the report as actioned. Pair with a listing suspend/cancel from /admin/listings.'
              : 'This dismisses the report — the listing remains visible.'
          }
