...
func TestMalformedUUIDPathGuardsAre400(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			role:    "admin",
			handler: NewAdminDisputesHandler(nil).GetDispute,
		},
		{
		...
		},
		{
			...
			role:    "provider",
			handler: NewContractHandler(nil).SubmitMilestone,
		},
		{
			...
			role:    "customer",
			handler: NewContractHandler(nil).ApproveMilestone,
		},
		{
			...
			handler: NewContractHandler(nil).RequestRevision,
		},
		{
			...
			role:    "customer",
			handler: NewContractHandler(nil).GetContract,
		},
		{
			...
			role:    "customer",
			body:    `{}`,
			handler: NewInstantMatchHandler(nil, nil, nil).CreateInstantMatch,
		},
		{
