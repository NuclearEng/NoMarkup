...
actor APIClient {
    ...
    /// GET `/api/v1/jobs?page=&page_size=&q=&category_ids=&latitude=&longitude=&radius_km=&schedule_type=&min_price_cents=&status=`
    ...
    /// - Parameter status: public Search filter (`open` aliases `active`). Browse/home pass `open`.
    func fetchJobs(
        page: Int = 1,
        pageSize: Int = 20,
        q: String? = nil,
