...
| C3.5 | Identity & license verification | `live` | web, ios | Provider: iOS `VerificationDocumentsView` + web `/provider/verification` (list+upload+FR-2.10 lockout) + licenses; admin review **web** (`/admin/verification`) — iOS `n/a-client` for admin | PII; 10 MB; server MIME | n/a | — |
