...
| Provider GPS check-in | Provider workspace; client lat/lng + server geo-fence (haversine vs job site, default 500 m / `CHECKIN_MAX_DISTANCE_METERS`; fail soft if no location) | Core Location | Location When In Use | — | shipped | not started | Include if provider role in binary; purpose string required | **5.1.5** · 2.5.1 |
