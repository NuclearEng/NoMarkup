...
**Recounted 2026-07-27** (ARC-10 Partial: sticky % + exposure metrics) (140 code/ops rows).
...
| P0 — Production deploy / ops | 6 | 2 | 19 | 1 | 0 | 28 |
| P1 — North Star performance | 0 | 3 | 8 | 5 | 0 | 16 |
| P1 — CI / testing enforcers | 1 | 0 | 10 | 5 | 0 | 16 |
...
| ID | Title | Sev | Area | Location | Action | Verify | Status | Owner |
|----|-------|-----|------|----------|--------|--------|--------|-------|
| FE-01 | axe-core only on HTML stubs; contrast disabled | BLOCKER | web | `tests/integration/axe.test.ts` + `tests/e2e/axe.spec.ts` | Playwright + axe on real routes; enable contrast | CI a11y gate | **Partial** 2026-07-27 — Playwright injects axe-core on `/` + `/marketplace` + `/jobs` + `/pricing` + `/login` with color-contrast ON (serious/critical fail the test); backend/web unloadable → skip not greenwash. `/login` is auth-shell only (no credentials/stack required). CookieConsent helper copy bumped zinc-500→400 so first real contrast hit is fixed. Residual (not Done): still no dashboard/admin surfaces; moderate/minor not gated; not full WCAG 2.2 AA cert; jsdom suite still disables contrast by design | |
