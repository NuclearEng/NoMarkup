...
**Recounted 2026-07-27** (OPS-24/25 Partial + FE-04 Done recount) (140 code/ops rows).
...
| P0 — Production deploy / ops | 0 | 8 | 19 | 1 | 0 | 28 |
...
| OPS-10 | Prometheus/Grafana/Alertmanager not in k8s | BLOCKER | k8s | deploy/monitoring only | Deploy stack or drop claim | Alerts fire on test | **Partial** 2026-07-27 — optional kustomize `deploy/monitoring/` (Prometheus + Alertmanager, not in `deploy/k8s/base`); reuses `prometheus.yml` + `alerts.yml` + new `alertmanager/alertmanager.yml`; SD scrape of `nomarkup` pods + `METRICS_BEARER_TOKEN` file mount; docs `docs/operations/monitoring-stack.md`. **Not Done:** no live cluster proof; emptyDir (no TSDB PVC); Alertmanager blackhole only; no kube-state-metrics (kube_* alerts dark); Grafana file-only; staging NS not in SD | |
...
| PERF-10 | k6 load tests not in CI; mock auth | MAJOR | tests/load | `tests/load/` | Nightly k6 against staging with real tokens | Artifacts + thresholds | **Partial** 2026-07-27 — `tests/load/smoke.js` (public GETs, 1 VU × 5 iter) + CI job `k6-smoke` on schedule/`workflow_dispatch` when repo var/secret `K6_BASE_URL` set (skip if unset; `continue-on-error`; JSON artifact). Full profiles still local; mock auth remains for authed scripts. **Not Done** without real staging load proof + tokens + capacity thresholds | |
...
| ID | Title | Sev | Area | Location | Action | Verify | Status | Owner |
|----|-------|-----|------|----------|--------|--------|--------|-------|
| FE-01 | axe-core only on HTML stubs; contrast disabled | BLOCKER | web | `tests/integration/axe.test.ts` + `tests/e2e/axe.spec.ts` | Playwright + axe on real routes; enable contrast | CI a11y gate | **Partial** 2026-07-27 — Playwright injects axe-core on public `/` + `/marketplace` + `/jobs` + `/pricing` + `/login` + `/register` with color-contrast ON (serious/critical fail). Protected `/dashboard` + `/settings/security`: unauth CI middleware-redirects to `/login?next=…` and we axe that login shell honestly (never claim dashboard chrome was scanned); with a session the target surface is axed. Backend/web unloadable → skip not greenwash. CookieConsent helper copy bumped zinc-500→400 so first real contrast hit is fixed. Residual (not Done): authenticated dashboard/admin chrome still not gated in default web-only CI (no session); moderate/minor not gated; not full WCAG 2.2 AA cert; jsdom suite still disables contrast by design | |
...
| FE-03 | Raw hex pervasive (~180 hits); “never hex” false | MAJOR | web | components + globals | Tokens + Tailwind brand colors; ban arbitrary hex in components | Lint rule / grep CI | **Partial** 2026-07-27 — CI ban shipped (`web/scripts/check-no-raw-hex.mjs` + `npm run check:hex` in `web-lint-typecheck`); worst offenders converted to CSS vars/tokens (PriceDropChart, BidDepthChart, SavingsCelebration, sparkline, GradientMesh→globals, MarketRangeDisplay, MarketTickerStrip, terminal-grid, LandingPageClient avatars/stars). Allowlist documented in `docs/design-system.md` § Raw hex policy (OG icons, themeColor, Google OAuth brand, Mapbox paint). Residual (not Done): allowlisted files still carry intentional hex; globals.css remains token SSOT with hex by design; not every historical comment/doc hex scrubbed outside the scanned trees | |
...
| ARC-10 | Experimentation: binary flags only | P2 | product | flags | % rollout + exposure logging | Experiment API | **Partial** 2026-07-27 — durable foundation, not full experiment platform: migration `115_feature_flags_rollout_percent` (`rollout_percent` 0-100 default 100); `RequireFlag` sticky SHA256(userID\|key)%100 for non-money keys; money/regulated keys binary-only (fail-closed on 1-99 + admin write reject); Prometheus `feature_flag_checks_total{flag,result}` + `experiment_exposures_total{key,variant}`; `WithExperiment` sticky control/treatment + fixed userID subject. Public `/api/v1/flags` stays flat bool (CDN). **Not Done:** multi-arm experiments, analysis warehouse/stats, admin UI for %, per-user public flag map | |
