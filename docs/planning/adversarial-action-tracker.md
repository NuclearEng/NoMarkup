...
**Recounted 2026-07-27** (OPS-02 Partial: terraform skeleton honesty) (140 code/ops rows).
...
| FE-04 | Loader2/spinners vs “Skeleton only” | MAJOR | web | many pages | Skeleton/ContentLoader for page load; spinner for button pending | Audit key routes | **Partial** 2026-07-27 — page-level loads now Skeleton/ContentLoader on key routes: AuthGuard, provider layout + ProviderRankCard + ProfessionalLicenseSection, /jobs browse + /jobs/map, /me/watchlist|/wishlist|/saved-searches|/feed, /orders list+detail, /marketplace browse + map side panel + spectate, /contracts/[id] page+review + `/contracts/[id]/review`, /insurance/quotes + /disputes/new Suspense, InstantAvailabilityCard initial fetch, admin/payments fee config + admin/platform growth/category panels; **plus residual list conversion** ChannelList, BidList, NotificationBell dropdown, FraudAlertList, public `/providers` grid (`Skeleton` shimmer, no ad-hoc `animate-pulse`). Keep intentional: button/action Loader2 (admin advances/insurance, contracts actions, InstantAvailability save, provider expense/tax/invoice mutations); installment “processing” status icon spinner; LIVE/urgency chrome pulse (BidCard/BidActivityFeed dots, CountdownClock, ListingBidPanel live highlight, spectate Wifi/Zap, BidVelocityIndicator). Residual (not Done): lower-traffic ad-hoc load placeholders still on LegalLanding, ListingDetailClient fallback, TrendingRail/SimilarListings cards, LegalIntakeForm field, PerformanceChart, auctions spectate Suspense shell | |
