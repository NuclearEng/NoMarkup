...
> **Honest status (ARC-12, 2026-07-27):** This section is **product strategy / roadmap**, not a shipped ML moat.
> Production fraud and pricing are **deterministic heuristics**; ONNX/`ort` is reserved for v2 and **off by
> default** (`ml/README.md`, CLAUDE.md §2). No trained model is served in the production path today. Do not
> cite “insurmountable ML moat,” “mature ML models,” or “AI-powered fraud” as current product state.
> Collecting settled non-PII transaction events for a future flywheel remains valid engineering intent.

### Overview

NoMarkup's long-term defensibility thesis is **compounding intelligence** from verified transactions — not the marketplace UI alone. Each completed job *can* make pricing more accurate, fraud detection more precise, and matching more effective **once models are trained and served**. That is the strategy target; it is **not** present capability. Timeline language below (12 / 24 months) is aspirational strategy, not a measured SLA.
